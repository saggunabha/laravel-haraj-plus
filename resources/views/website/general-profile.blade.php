    ...
    <script type="application/ld+json">
        {
            ...
            "disambiguatingDescription": "{{$user->about}}"
        }
        
        </script>
@endsection
@section('seoTitle', $user->user->name)
@section('seoDescription', strip_tags($user->about) )
@section('seoKeywords', ($user->user->name))
@section('seoGoogleImg', asset('storage/'.$user->user->image))
@section('content')
                ...
                <div class="row">
                    ...
                    <div class="col-12 text-center wow fadeIn">
                        <div class="store-information-text">
                            <h2 class="info-title first_color">نبذة عن {{$user->user->name}}</h2>
                            <p>
                                {{$user->about}}
                            </p>
                        </div>
                    ...
                    <div class="col-12 two-btns  margin-div text-left-dir wow fadeIn">
                        ...
                        <a  href="{{route("chat.getChat",$user->user->id)}}"  @if(!auth()->id())   data-toggle="modal" data-target="#login-modal"   @endif class="custom-btn big-btn">راسلنا</a>
                        <br>
                         <?php $phone =  format_number($user->user->phone) ;?>
                        <a  href="https://api.whatsapp.com/send?phone={{$phone}}"  class="custom-btn big-btn">للتواصل عبر الواتس اب</a>
                                ...
                                <div class="product-div">
                                    @if(auth()->id())
                                        @if($favouriteProducts!=[] )
                                            ...
                                            <span @if(in_array($product->id,$favouriteProducts)) class="fav-icon first_bg active" @else  class="fav-icon first_bg " @endif  ><i class="far fa-heart"></i></span>
                                        @else
...
<script type='text/javascript' src='https://platform-api.sharethis.com/js/sharethis.js#property=5e9e29a0e3a49d0012075ee9&product=image-share-buttons&cms=website' async='async'></script>
    ...
    $(function () {
    $(".load-div").slice(0, 8).fadeTo("slow", 1);

    $("#loadmore").on('click', function (e) {
    e.preventDefault();
    $(".load-div:hidden").slice(0, 8).slideDown("fast");
