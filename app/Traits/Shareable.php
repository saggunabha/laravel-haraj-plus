    ...
    public function getShareUrl($type = 'facebook')
    {
        ...
        if ($type == 'facebook') {
            $query = urldecode(http_build_query([
                ...
                'href' => $url,
                'display' => "page",
                'title' => urlencode($this->{array_get($this->shareOptions, 'columns.name','columns.description')})
            ]));
        ...
        if ($type == 'twitter') {
            $query = urldecode(http_build_query([
                'url' => $url,
                'text' => urlencode(str_limit($this->{array_get($this->shareOptions, 'columns.name')}, 120))
            ]));
        ...
        if ($type == 'whatsapp') {
            $query = urldecode(http_build_query([
                'text' => urlencode($this->{array_get($this->shareOptions, 'columns.name')} . ' ' . $url)
            ]));
        ...
        if ($type == 'linkedin') {
            $query = urldecode(http_build_query([
                'url' => $url,
                'summary' => urlencode($this->{array_get($this->shareOptions, 'columns.name')})
            ]));
        ...
        if ($type == 'pinterest') {
            $query = urldecode(http_build_query([
                'url' => $url,
                'description' => urlencode($this->{array_get($this->shareOptions, 'columns.name')})
            ]));
